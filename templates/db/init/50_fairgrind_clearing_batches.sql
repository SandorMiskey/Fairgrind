...
CREATE TABLE `{{.DB_SCHEMA}}`.`clearing_batch_types` (
  ...
  PRIMARY KEY (`id`),
  KEY `clearing_batch_types_deleted_at_idx` (`deleted_at`) USING BTREE,
  CONSTRAINT `clearing_batch_types_projects_fk` FOREIGN KEY (`project_id`) REFERENCES `projects` (`id`) ON UPDATE CASCADE
) {{.DB_TABLE_OPTIONS}};

INSERT INTO {{.DB_SCHEMA}}.clearing_batch_types 
  (project_id,label,description,multiplier)
VALUES
  ({{.DB_TEMPLATE_PROJECT}}, 'Exam', 'To be used during the testing or training of a Grinder', 0.0),
  ({{.DB_TEMPLATE_PROJECT}}, 'Test', 'To be Used When Examining the efficiency and/or clarity of an annotation task', 1.0),
  ({{.DB_TEMPLATE_PROJECT}}, 'Production', 'Production batch', 1.0);
...
INSERT INTO {{.DB_SCHEMA}}.clearing_batches
  (clearing_batch_type_id, clearing_batch_status_id, label, description)
VALUES
  (1, 2, 'Exam template #1', 'Clearing template batch: type=Exam, status=In progress'),
  (1, 3, 'Exam template #2', 'Clearing template batch: type=Exam, status=Suspended'),
  (1, 5, 'Exam template #3', 'Clearing template batch: type=Exam, status=Cleared'),
  (2, 2, 'Test template #1', 'Clearing template batch: type=Test, status=In progress'),
  (2, 3, 'Test template #2', 'Clearing template batch: type=Test, status=Suspended'),
  (2, 5, 'Test template #3', 'Clearing template batch: type=Test, status=Cleared'),
  (3, 2, 'Prod template #1', 'Clearing template batch: type=Production, status=In progress'),
  (3, 3, 'Prod template #2', 'Clearing template batch: type=Production, status=Suspended'),
  (3, 5, 'Prod template #3', 'Clearing template batch: type=Production, status=Cleared');
